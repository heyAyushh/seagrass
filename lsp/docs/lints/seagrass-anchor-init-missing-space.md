# Anchor Init Missing Space

Topic: `seagrass/anchor.init.missing-space`

Source: `seagrass`

## What It Catches

Anchor init constraints missing account space evidence.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.init.missing-space` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor init missing space | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.init.missing-space
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.init.missing-space
```
...
```toml
[lints]
allow = ["seagrass/anchor.init.missing-space"]
```
