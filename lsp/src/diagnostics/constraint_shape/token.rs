use {
    super::support::constraint_uses_family,
    crate::{
        constraint_catalog::ConstraintFamily,
...
pub(super) fn constraint_diagnostics(
    ...
    diagnostics.extend(token_constraint_type_diagnostic(field, constraint));
    diagnostics.extend(mint_constraint_type_diagnostic(field, constraint));
    diagnostics
}
fn token_constraint_type_diagnostic(
    field: &FieldEvidence<'_>,
    constraint: &ConstraintEvidence<'_>,
) -> Option<Diagnostic> {
    let uses_token_constraint = constraint_uses_family(
        constraint,
        [
    ...
    if !uses_token_constraint || field.has_generic_type("TokenAccount") {
        return None;
    }
    ...
    Some(diagnostic_from_range(
        ...
        format!(
            "`{}` uses token account constraints but is not typed `{}`.",
            field.field.name,
            expected_account_wrapper(field, "TokenAccount")
        ),
        Some(serde_json::json!({
            ...
            "expected": expected_account_wrapper(field, "TokenAccount"),
        })),
    ))
}
fn mint_constraint_type_diagnostic(
    field: &FieldEvidence<'_>,
    constraint: &ConstraintEvidence<'_>,
) -> Option<Diagnostic> {
    let uses_mint_constraint = constraint_uses_family(
        constraint,
        [ConstraintFamily::Mint, ConstraintFamily::MintExtension],
    );
    if !uses_mint_constraint || field.has_generic_type("Mint") {
        return None;
    }
    ...
    Some(diagnostic_from_range(
        ...
        AnchorDiagnosticKind::AnchorConstraintShape,
        format!(
            "`{}` uses mint constraints but is not typed `{}`.",
            field.field.name,
            expected_account_wrapper(field, "Mint")
        ),
        Some(serde_json::json!({
            "account": field.field.name,
            "constraint": "mint",
            "quickfix": "replace-account-type",
            "anchorError": "ConstraintMintDecimals",
            "expected": expected_account_wrapper(field, "Mint"),
        })),
    ))
}
fn expected_account_wrapper(field: &FieldEvidence<'_>, generic: &str) -> String {
