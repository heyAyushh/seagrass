...
pub(super) fn constraint_uses_family<const N: usize>(
    constraint: &ConstraintEvidence<'_>,
    families: [ConstraintFamily; N],
) -> bool {
    families
        .into_iter()
        .any(|family| constraint_catalog::family_keys(family).any(|key| constraint.has_key(key)))
}
