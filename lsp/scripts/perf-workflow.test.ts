...
describe("perf workflow guardrails", () => {
  test("runs hotpath replay with budget enforcement", () => {
    ...
    expect(workflow).toContain("bun lsp/scripts/perf-replay.ts");
  ...
  test("publishes scale reports for the 10/50/200 workspace sweep", () => {
    ...
    expect(workflow).toContain("bun lsp/scripts/scale-benchmark.ts");
  ...
  test("keeps perf workflow drift in PR and local production guardrails", () => {
    ...
    expect(prWorkflow).toContain("bun test lsp/scripts/perf-workflow.test.ts");
    expect(verifyProduction).toContain('"lsp/scripts/perf-workflow.test.ts"');
