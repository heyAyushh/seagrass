...
const TOKEN_ACCOUNT_MEMBERS: &[&str] = &[
    "amount",
    "close_authority",
    "delegate",
    "delegated_amount",
...
impl ResolvedAccountType {
    pub fn display_name(self) -> &'static str {
        match self {
            ...
            Self::DataStruct => "account data",
            Self::Composite => "composite account",
            Self::Unknown => "account",
        }
    }

    pub fn allows_constraint_family(self, family: ConstraintFamily) -> bool {
        match self {
            Self::Mint => matches!(
                family,
            ...
            | Self::DataStruct
            | Self::Composite => matches!(
                family,
                ConstraintFamily::Core | ConstraintFamily::Pda | ConstraintFamily::Realloc
            ),
            Self::Unknown => true,
        }
    }
...
pub fn resolved_account_member_names(account_type: ResolvedAccountType) -> &'static [&'static str] {
    match account_type {
        ResolvedAccountType::Mint => MINT_MEMBERS,
        ResolvedAccountType::TokenAccount => TOKEN_ACCOUNT_MEMBERS,
        ResolvedAccountType::Program
        | ResolvedAccountType::Signer
        | ResolvedAccountType::SystemAccount
        | ResolvedAccountType::Sysvar
        | ResolvedAccountType::UncheckedAccount
        | ResolvedAccountType::DataStruct
        | ResolvedAccountType::Composite
        | ResolvedAccountType::Unknown => &[],
    }
}

pub fn resolve_field_account_type(
    accounts: &SymbolRange,
    field: &SymbolRange,
) -> ResolvedAccountType {
    let type_name = field.type_name.as_deref();
    let inner = declared_or_expected_account_inner_type(accounts, field);
...
fn resolve_field_type_parts(
    field: &SymbolRange,
    type_name: Option<&str>,
    inner: Option<&str>,
) -> ResolvedAccountType {
    match (type_name, inner) {
        (Some("Account" | "InterfaceAccount" | "LazyAccount" | "AccountLoader"), Some("Mint")) => {
            ResolvedAccountType::Mint
        }
        (
            Some("Account" | "InterfaceAccount" | "LazyAccount" | "AccountLoader"),
            Some("TokenAccount"),
        ) => ResolvedAccountType::TokenAccount,
        (Some("Program" | "Interface"), _) => ResolvedAccountType::Program,
        (Some("Signer"), _) => ResolvedAccountType::Signer,
        (Some("SystemAccount"), _) => ResolvedAccountType::SystemAccount,
        (Some("Sysvar"), _) => ResolvedAccountType::Sysvar,
        (Some("AccountInfo" | "UncheckedAccount"), _) => ResolvedAccountType::UncheckedAccount,
        (Some("Account" | "InterfaceAccount" | "LazyAccount" | "AccountLoader"), Some(_)) => {
            ResolvedAccountType::DataStruct
        }
        (Some(_), None) if field.generic_type_names.is_empty() => ResolvedAccountType::Composite,
        _ => ResolvedAccountType::Unknown,
    }
}
