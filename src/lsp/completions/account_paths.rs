...
fn account_loader_method_candidates(owner_type: &str) -> Vec<FieldCandidate> {
    account_members::ACCOUNT_LOADER_LOADED_METHOD_COMPLETIONS
        .iter()
        .map(|method| FieldCandidate {
            name: (*method).to_string(),
            detail: format!("{owner_type} method"),
...
fn local_container_after_segments(
    ...
    for (index, segment) in completed_segments.iter().enumerate() {
        ...
        if let Some(loader_type) = account_members::account_loader_type_name_from_parts(
            field.type_name.as_deref(),
            &field.generic_type_names,
        ) {
            return is_last.then_some(FieldContainer::AccountLoader(loader_type));
        }
...
fn field_shape_for_segment(
    ...
    match container {
        ...
        FieldContainer::AccountLoader(_) => None,
    }
}
...
fn next_container_for_field(
    ...
    if let Some(loader_type) = account_members::account_loader_type_name_from_parts(
        ...
        return Some(FieldContainer::AccountLoader(loader_type));
    }
