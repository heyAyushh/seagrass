...
#[test]
fn completes_associated_values_through_type_alias() {
    let source = r#"
use crate::state::State as Ledger;

#[derive(Accounts)]
pub struct Create<'info> {
    #[account(init, payer = payer, space = Ledger::SP)]
    pub state: Account<'info, State>,
    #[account(mut)]
    pub payer: Signer<'info>,
}
...
"#;
    let document = ParsedDocument::parse(source).unwrap();
    let completions = completions(&document, position_after(source, "space = Ledger::SP"))
        .expect("aliased associated space completions");
