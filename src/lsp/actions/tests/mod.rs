...
#[test]
fn proactive_assist_range_participates_in_cursor_ranking() {
    let cursor = point_range(Position {
        line: 12,
        character: 4,
    });
    let far_diagnostic = Diagnostic {
        range: Range {
            start: Position {
                line: 2,
                character: 0,
    ...
    let diagnostic_action = CodeAction {
        title: "Far diagnostic quickfix".to_string(),
        diagnostics: Some(vec![far_diagnostic]),
        ..CodeAction::default()
    };
    let proactive_action = CodeAction {
        title: "Nearby proactive assist".to_string(),
        data: Some(serde_json::json!({
            "seagrassAssist": "test-assist",
            "seagrassAssistRange": {
    ...
    let ranked = rank_and_filter_for_cursor(vec![diagnostic_action, proactive_action], cursor);

    assert_eq!(
        ranked.first().map(|action| action.title.as_str()),
        Some("Nearby proactive assist")
    );
}
