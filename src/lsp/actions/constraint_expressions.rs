...
use {
    ...
    crate::diagnostics::{
        ANCHOR_CONSTRAINT_EXPRESSION_CODE, REPLACE_CONSTRAINT_EXPRESSION_MEMBER_QUICKFIX,
    },
...
pub fn code_actions(uri: Url, _range: Range, diagnostics: &[Diagnostic]) -> Vec<CodeAction> {
    diagnostics
        ...
        .filter_map(|diagnostic| member_replacement_action(uri.clone(), diagnostic))
        .collect()
}
...
fn member_replacement_action(uri: Url, diagnostic: &Diagnostic) -> Option<CodeAction> {
    ...
    let replacement = closest_member_candidate(data, missing)?;
...
fn closest_member_candidate<'a>(data: &'a serde_json::Value, missing: &str) -> Option<&'a str> {
    data.get("candidates")
