...
#[test]
fn accepts_trait_extension_method_without_inherent_candidates() {
    let document = ParsedDocument::parse_or_empty(
        r#"
use anchor_lang::prelude::*;
...
trait PositionBundleExt {
    fn extension_method(&self) -> bool;
}

impl PositionBundleExt for PositionBundle {
    fn extension_method(&self) -> bool {
        true
    }
}
"#,
    ...
    assert!(
        diagnostics.iter().all(|diagnostic| {
            !diagnostic
                .message
                .contains("`bundle.extension_method()` does not resolve")
        }),
        "trait extension methods should stay outside shallow method diagnostics: {diagnostics:#?}"
    );
}
