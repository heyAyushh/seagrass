use {
    super::slots::ConstraintValueSlot,
    crate::{
        account_semantics,
...
pub(super) fn value_items_for_slot(
    ...
    match slot.value_kind {
        ...
        ConstraintValueKind::Space => space_items(accounts, current_field),
        ConstraintValueKind::Seeds => seed_items(document, accounts, current_field),
        ConstraintValueKind::None | ConstraintValueKind::AnyExpression => Vec::new(),
    }
}
...
fn has_one_items(
    ...
    let account_data_fields =
        account_data_field_names(document, workspace_index, account_data_type);
...
fn account_data_field_names(
    ...
    workspace_index
        .map(|index| index.field_names_in_container(account_data_type))
        .unwrap_or_default()
        .into_iter()
        .collect()
}
