# Anchor Artifact Types

Topic: `seagrass/anchor.artifact.types`

Source: `seagrass`

## What It Catches

Anchor generated TypeScript type artifact diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.artifact.types` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor artifact types | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.artifact.types
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.artifact.types
```
...
```toml
[lints]
allow = ["seagrass/anchor.artifact.types"]
```
