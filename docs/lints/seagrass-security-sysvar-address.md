# Security Sysvar Address

Topic: `seagrass/security.sysvar.address`

Source: `seagrass`

## What It Catches

Sysvar address validation diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/security.sysvar.address` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies security sysvar address | diagnostic |
...
```rust
// seagrass-allow: seagrass/security.sysvar.address
```

File suppression:

```rust
// seagrass-allow-file: seagrass/security.sysvar.address
```
...
```toml
[lints]
allow = ["seagrass/security.sysvar.address"]
```
