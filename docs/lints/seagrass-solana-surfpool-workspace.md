...
Topic: `seagrass/solana.surfpool-workspace`

Source: `seagrass`

## What It Catches

Surfpool workspace diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/solana.surfpool-workspace` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies solana surfpool workspace | diagnostic |
...
```rust
// seagrass-allow: seagrass/solana.surfpool-workspace
```

File suppression:

```rust
// seagrass-allow-file: seagrass/solana.surfpool-workspace
```
...
```toml
[lints]
allow = ["seagrass/solana.surfpool-workspace"]
```
