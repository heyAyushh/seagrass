# Anchor Init Missing Companion

Topic: `seagrass/anchor.init.missing-companion`

Source: `seagrass`

## What It Catches

Anchor init companion constraint diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.init.missing-companion` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor init missing companion | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.init.missing-companion
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.init.missing-companion
```
...
```toml
[lints]
allow = ["seagrass/anchor.init.missing-companion"]
```
