...
Topic: `seagrass/anchor.spl-token-interface`

Source: `seagrass`

## What It Catches

Anchor SPL token and token-interface semantic diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.spl-token-interface` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor spl token interface | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.spl-token-interface
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.spl-token-interface
```
...
```toml
[lints]
allow = ["seagrass/anchor.spl-token-interface"]
```
