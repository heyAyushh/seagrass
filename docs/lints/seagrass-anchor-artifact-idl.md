...
Topic: `seagrass/anchor.artifact.idl`

Source: `seagrass`

## What It Catches

Anchor IDL artifact freshness and availability diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.artifact.idl` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor artifact idl | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.artifact.idl
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.artifact.idl
```
...
```toml
[lints]
allow = ["seagrass/anchor.artifact.idl"]
```
