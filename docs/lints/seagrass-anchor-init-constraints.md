...
Topic: `seagrass/anchor.init.constraints`

Source: `seagrass`

## What It Catches

General Anchor init constraint diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.init.constraints` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor init constraints | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.init.constraints
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.init.constraints
```
...
```toml
[lints]
allow = ["seagrass/anchor.init.constraints"]
```
