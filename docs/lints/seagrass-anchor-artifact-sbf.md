...
Topic: `seagrass/anchor.artifact.sbf`

Source: `seagrass`

## What It Catches

Anchor SBF build artifact diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.artifact.sbf` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor artifact sbf | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.artifact.sbf
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.artifact.sbf
```
...
```toml
[lints]
allow = ["seagrass/anchor.artifact.sbf"]
```
