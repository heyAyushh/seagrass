# Anchor Instruction Argument

Topic: `seagrass/anchor.instruction.argument`

Source: `seagrass`

## What It Catches

Anchor instruction attribute argument diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.instruction.argument` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor instruction argument | diagnostic |
...
```rust
// seagrass-allow: seagrass/anchor.instruction.argument
```

File suppression:

```rust
// seagrass-allow-file: seagrass/anchor.instruction.argument
```
...
```toml
[lints]
allow = ["seagrass/anchor.instruction.argument"]
```
