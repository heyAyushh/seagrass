...
Topic: `seagrass/security.token-account`

Source: `seagrass`

## What It Catches

Token account validation security diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/security.token-account` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies security token account | diagnostic |
...
```rust
// seagrass-allow: seagrass/security.token-account
```

File suppression:

```rust
// seagrass-allow-file: seagrass/security.token-account
```
...
```toml
[lints]
allow = ["seagrass/security.token-account"]
```
