...
Topic: `seagrass/anchor.constraint.shape`

Source: `seagrass`

## What It Catches

Anchor account constraint parser and shape diagnostics.
...
| fixture | expected |
| --- | --- |
| `seagrass/anchor.constraint.shape` appears only in a comment, doc comment, or string literal | no diagnostic |
...
| parsed evidence satisfies anchor constraint shape | diagnostic |
...
```rust
// seagrass-allow-file: seagrass/anchor.constraint.shape
```
...
```toml
[lints]
allow = ["seagrass/anchor.constraint.shape"]
```
