...
| `lsp/diagnostics/handler_scope.rs` | unresolved handler value identifiers | yes, parsed handler bodies plus shallow local scopes | hot lane, handler/helper bodies only | derived | `seagrass/anchor.account.usage` | fixed: call names, imports, non-Anchor functions | `hot_engine_flags_unresolved_handler_identifiers`, `editor_ux_flags_unresolved_anchor_handler_identifier` |
