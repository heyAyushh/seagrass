...
| `lsp/diagnostics/handler_members.rs` | handler local member resolution | yes, parsed handler bodies plus shallow local/argument/alias types | hot lane, handler/helper bodies only | derived | `seagrass/anchor.account.usage` | fixed: unknown external types stay outside shallow resolver | `reports_generated_unknown_member_through_typed_alias`, `completes_generated_typed_handler_alias_members` |
