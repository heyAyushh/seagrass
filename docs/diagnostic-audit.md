...
| `lsp/diagnostics/handler_scope.rs` | unresolved handler value identifiers | yes, parsed handler bodies plus shallow local scopes | handler/helper bodies only | derived | `seagrass/anchor.account.usage` | fixed: call names, imports, non-Anchor functions | `editor_ux_flags_unresolved_anchor_handler_identifier` |
...
| `lsp/completions/account_paths.rs` | `ctx.accounts.*` path completions | parsed account path + workspace index | instruction body | n/a | n/a | low | account path tests |
