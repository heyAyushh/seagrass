...
| `lsp/diagnostics/constraint_expressions.rs` | account constraint expression resolution | yes, parsed expressions plus account/member/method/workspace symbols and typed instruction-argument members | account attribute expression spans | authoritative | `seagrass/anchor.constraint.expression` | low | constraint expression tests |
...
| `lsp/completions/constraint_values/members.rs` | constraint expression member completions | account/member resolver, typed instruction arguments, and workspace symbols | account attribute expression values | n/a | n/a | low | semantic constraint value tests |
...
| `lsp/diagnostics/constraint_expressions/instruction_args.rs` | typed instruction-argument member resolution | parsed instruction arg types plus shared struct-member resolver | account attribute expression spans | authoritative | `seagrass/anchor.constraint.expression` | low | `validates_instruction_argument_struct_members`, `reports_generated_unknown_instruction_argument_members` |
