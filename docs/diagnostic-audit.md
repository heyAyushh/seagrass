...
| `lsp/diagnostics/handler_struct_literals.rs` | handler struct literal field resolution | yes, `syn::visit::Visit` over `ExprStruct` plus shared struct/member resolver | hot lane, Anchor handler bodies only | derived | `seagrass/anchor.account.usage` | low | `reports_unknown_handler_struct_literal_field`, `ignores_unknown_external_struct_literal_type`, `editor_ux_resolves_handler_struct_literal_fields` |
...
| `lsp/completions/handler_struct_fields.rs` | handler struct literal field completions | parsed struct/member resolver plus bounded text recovery for incomplete record literals | Anchor handler struct literal field cursor slots | n/a | n/a | low | handler struct literal completion tests + editor parity |
| `lsp/completions/handler_values.rs` | handler expression value completions | parsed handler scope, account context fields, imports, constants, and assertion macro empty argument slots | handler body cursor expression values | n/a | n/a | low | handler value tests + generated proptest |
