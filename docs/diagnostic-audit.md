...
| `lsp/diagnostics/handler_members.rs` | handler local member resolution | yes, parsed handler bodies plus shallow local/argument types | hot lane, handler/helper bodies only | derived | `seagrass/anchor.account.usage` | fixed: unknown external types stay outside shallow resolver | `reports_generated_unknown_typed_handler_member`, `completes_generated_typed_handler_members` |
