...
| `lsp/completions/handler_members.rs` | handler member completions | parsed handler local/argument types, `Context<T>` account aliases, intermediate `ctx.accounts` aliases, generated `Bumps` PDA fields, inherent impl methods, and workspace symbols | Anchor handler member access | n/a | n/a | low | handler member tests + generated proptest |
